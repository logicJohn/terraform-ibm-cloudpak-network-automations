locals {
  ...
  orchestration           = file(join("/", [path.module, "files", "orchestration.yaml"]))
}
...
resource "null_resource" "install_cp4na" {
  ...
  triggers = {
    ...
    orchestration                = sha1(local.orchestration)
  }
...
data "external" "get_cp4na_endpoints" {
  depends_on = [
    null_resource.install_cp4na
  ]

  program = ["/bin/bash", "${path.module}/scripts/get_cp4na_endpoints.sh"]
